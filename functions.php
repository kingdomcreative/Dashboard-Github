...
function fetchStaffEmails() {
		...
		//Can't connect to DB
		if( mysqli_connect_errno() )
			throw new Exception('Could not connect to database (function: fetchStaffEmail).');
			
	      $dbEmailQuery = 'SELECT * FROM staff WHERE _kp_STAFF <> 11;';
					
			$dbEmailResult = $dbLink->query($dbEmailQuery);
...
function fetchClipboardData() {
		...
		//Can't connect to DB
		if( mysqli_connect_errno() )
			throw new Exception('Could not connect to database (function: fetchClipboardData).');
			
			
	      $dbClipboardQuery = 'SELECT *, DATE_FORMAT(_date_CLIPBOARD, "%e %b %Y") AS date_CLIPBOARD FROM staff_clipboard LEFT JOIN staff ON staff_clipboard._kf_staff_CLIPBOARD = staff._kp_STAFF ORDER BY _date_CLIPBOARD DESC';
					
			$dbClipboardResult = $dbLink->query($dbClipboardQuery);
			
			//General Query error
			if( $dbLink->errno )
				throw new Exception('An error occurred when fetching staff emails');
			
			return $dbClipboardResult;	
...
function fetchProjectForClient($clientID) {
		 ...
		 try {
			...
			$dbProjectQuery = 'SELECT *, DATE_FORMAT(_added_PROJECT, "%e %b %Y") AS added_PROJECT FROM project LEFT JOIN client ON project._kf_client_PROJECT = client._kp_CLIENT WHERE _kf_client_PROJECT = "'.$clientID.'" ORDER BY _added_PROJECT DESC';
...
function fetchTodayFeed($date) {
		...
		if( mysqli_connect_errno() )
	      ...
	      $dbTodayQuery = 'SELECT * FROM today WHERE _date_TODAY LIKE "%'.$dbLink->real_escape_string($date).'%" ORDER BY _date_TODAY DESC LIMIT 4';
					
			$dbTodayResult = $dbLink->query($dbTodayQuery);
			
			//User details not found in DB	
			   if( $dbTodayResult->num_rows == 0 )
					return false;
			
			//General Query error
			if( $dbLink->errno )
				throw new Exception('An error occurred when querying the database for TODAY data');
			
				
			return $dbTodayResult;
